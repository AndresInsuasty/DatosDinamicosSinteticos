...
class PlantillaVentasEcommerce(PlantillaBase):
    ...
    def descripcion(self) -> str:
        return ("Datos de ventas online con métricas de conversión, "
                "tráfico y análisis de comportamiento")
    ...
    def generar(self) -> pd.DataFrame:
        ...
        dataframe['roi'] = ((dataframe['valor_pedido'] - dataframe['costo_adquisicion']) /
                           dataframe['costo_adquisicion'] * 100).round(2)

        # Aplicar nulos si es necesario (excluir IDs y fechas críticas)
        dataframe = self._aplicar_nulos(dataframe,
                                       columnas_excluir=['fecha_pedido', 'pedido_id', 'cliente_id'])
