...
class PlantillaRecursosHumanos(PlantillaBase):
    ...
    def descripcion(self) -> str:
        return ("Datos de empleados con salarios, departamentos, "
                "evaluaciones y métricas de RRHH")
    ...
    def generar(self) -> pd.DataFrame:
        ...
        data = {
            ...
            'nivel': np.random.choice(niveles, self.num_filas,
                                    p=[0.3, 0.25, 0.2, 0.1, 0.1, 0.05]),
            'tipo_contrato': np.random.choice(tipos_contrato, self.num_filas,
                                            p=[0.7, 0.15, 0.1, 0.05]),
        ...
        dataframe['antiguedad_anos'] = ((pd.Timestamp.now() -
                                       pd.to_datetime(dataframe['fecha_ingreso'])).dt.days / 365.25).round(1)

        # Aplicar nulos si es necesario (excluir IDs y fechas críticas)
        dataframe = self._aplicar_nulos(dataframe, columnas_excluir=['empleado_id', 'fecha_ingreso'])
